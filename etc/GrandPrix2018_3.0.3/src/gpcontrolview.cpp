...
GPControlView::GPControlView(QWidget *parent)
    : QWidget(parent), ui(new Ui::GPControlView), gpControl(NULL) {
  ui->setupUi(this);
  QObject::connect(ui->gpMapSelector, SIGNAL(mapSelected(QString)), this,
                   SLOT(on_gpMapSelector_mapSelected(QString)));
  ...
  connect(ui->cbFollow, SIGNAL(currentIndexChanged(int)), this,
          SLOT(onFollowModeChanged(int)));
  ...
  connect(ui->cbNextCarMode, SIGNAL(currentIndexChanged(int)), this,
          SLOT(onNextCarModeChanged(int)));

  newGPControl();
  enableControls();
}

GPControlView::~GPControlView() { delete ui; }
...
void GPControlView::newGPControl() {
  ...
  gpControl->setNextCarMode(static_cast<NextCarMode>(
      ui->cbNextCarMode->itemData(ui->cbNextCarMode->currentIndex()).toInt()));
...
void GPControlView::on_runbutton_clicked() {
  ...
  disableControls();
  QObject::connect(gpControl, SIGNAL(end(QString)), this,
                   SLOT(gpControl_end(QString)));
  QObject::connect(gpControl, SIGNAL(carMoved()), this,
                   SLOT(gpControl_carMoved()));
  QObject::connect(gpControl, SIGNAL(carMovedWithBoost()), this,
                   SLOT(gpControl_carMovedWithBoost()));
  QObject::connect(gpControl, SIGNAL(invalidMove()), this,
                   SLOT(gpControl_invalidMove()));
  QObject::connect(gpControl, SIGNAL(syntaxError()), this,
                   SLOT(gpControl_syntaxError()));
  QObject::connect(gpControl, SIGNAL(driverTimeout()), this,
                   SLOT(gpControl_driverTimeout()));
...
void GPControlView::gpControl_carMoved() {
  ...
  QString res =
      tr("Driver %1: move to (%2,%3) new velocity (%4,%5), %6 boosts left, gas "
         "left %7 (%8 used)")
          .arg(curCar + 1)
...
void GPControlView::gpControl_carMovedWithBoost() {
  ...
  int curCar = gpControl->getCurrentCar();
  QString res =
      tr("Driver %1 boost: move to (%2,%3) new velocity is (%4,%5), %6 boosts "
         "left, gas left %7 (%8 used)")
          .arg(curCar + 1)
          .arg(gpControl->getPosCarX(curCar))
          .arg(gpControl->getPosCarY(curCar))
          .arg(gpControl->getVelCarX(curCar))
...
void GPControlView::gpControl_invalidMove() {
  int curCar = gpControl->getCurrentCar();
  emit invalidMove(tr("Driver %1: invalid move %2 carburant")
                       .arg(gpControl->getCurrentCar() + 1)
                       .arg(gpControl->getCarburant(curCar)));
  updateDriverInfo();
}
...
void GPControlView::gpControl_syntaxError() {
  emit syntaxError(
      tr("Driver %1: syntax error").arg(gpControl->getCurrentCar() + 1));
}

void GPControlView::gpControl_driverTimeout() {
  emit driverTimeout(
      tr("Driver %1: time out.").arg(gpControl->getCurrentCar() + 1));
}
...
void GPControlView::onNextCarModeChanged(int index) {
  gpControl->setNextCarMode(
      static_cast<NextCarMode>(ui->cbNextCarMode->itemData(index).toInt()));
}
