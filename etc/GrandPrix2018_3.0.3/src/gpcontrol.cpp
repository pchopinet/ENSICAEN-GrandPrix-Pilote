...
GPControl::GPControl(QObject *parent)
    : QObject(parent), tiles(QVector<GPControl::Tile>()), cars(QVector<Car>(3)),
      nextCarMode(RANDOM), nbCars(0), delay(1) {
  QObject::connect(&timer, SIGNAL(timeout()), this, SLOT(timeout()));
  finishPosition = 1;
  currentCarIndex = 0;
}

GPControl::~GPControl() { timer.stop(); }
...
int GPControl::getSizeX() { return sizeX; }
int GPControl::getSizeY() { return sizeY; }
...
int GPControl::getNbCars() { return nbCars; }
int GPControl::getPosCarX(int noCar) { return cars[noCar].positionsX.last(); }
int GPControl::getPosCarY(int noCar) { return cars[noCar].positionsY.last(); }
int GPControl::getVelCarX(int noCar) { return cars[noCar].velX; }
int GPControl::getVelCarY(int noCar) { return cars[noCar].velY; }

int GPControl::getCarburant(int noCar) { return cars[noCar].gasoline; }
...
int GPControl::getBoost(int noCar) { return cars[noCar].boosts; }
...
int GPControl::getNbMoves(int noCar) { return cars[noCar].nbMoves; }
int GPControl::getNbFailures(int noCar) { return cars[noCar].nbFailures; }
int GPControl::getNbPositions(int noCar) {
  return cars[noCar].positionsX.size();
}
int GPControl::getPosDepart(int noCar) { return cars[noCar].position; }
QColor GPControl::getColor(int noCar) { return cars[noCar].color; }
int GPControl::getCurrentCar() { return currentCarIndex; }
void GPControl::setCarDebug(int car, bool debug) { cars[car].debug = debug; }
bool GPControl::getCarDebug(int car) { return cars[car].debug; }
QString GPControl::getDriverName(int car) { return cars[car].name; }
...
void GPControl::setDriverPath(int noCar, const QString &driverPath) {
  ...
  driver->start(driverPath,
                QIODevice::Unbuffered | QIODevice::Text | QIODevice::ReadWrite);
  QObject::connect(driver, SIGNAL(readyReadStandardError()), this,
...
void GPControl::setDelay(int value) { delay = value; }

QByteArray GPControl::mapData() { /* indique également le carburant de départ
...
QByteArray GPControl::positionsData() {
  ...
  for (int i = 0; i < nbCars; i++) {
    ...
    res.append(
        QString("%1 %2").arg(car.positionsX.last()).arg(car.positionsY.last()));
    if (i != nbCars - 1) {
      res.append("\t");
    }
  }
...
int GPControl::gasolineAtStart() const { return carburantAuDepart; }
...
void GPControl::nextMove() {
  ...
  QObject::connect(car.driver, SIGNAL(readyReadStandardOutput()), this,
...
/* Calcul de la variation de carburant en fonction de l'accéleration et de la
 * vitesse */
int GPControl::gasolineConsumption(int accX, int accY, int velX, int velY) {
  return accX * accX + accY * accY +
         (int)(sqrt(velX * velX + velY * velY) * 3.0 / 2.0);
}
...
void GPControl::moveCurrentCar(int accX, int accY) {
  ...
  if (getTile(car.positionsX.last(), car.positionsY.last()) ==
      GPControl::SAND) {
    gasUsed += 1;
  }
  ...
  for (int accX = -maxAcc; accX <= maxAcc; accX++) {
    for (int accY = -maxAcc; accY <= maxAcc; accY++) {
      if (getTile(posX, posY) == GPControl::NORMAL) {
        if (this->isFree(posX + velX + accX, posY + velY + accY) &&
            abs(velX + accX) * abs(velX + accX) +
                    abs(velY + accY) * abs(velY + accY) <=
                25) {
...
void GPControl::setNextCarMode(NextCarMode mode) { this->nextCarMode = mode; }
...
void GPControl::nextCar() {
  ...
  if (nextCarMode == RANDOM) {
    ...
    if (!positionInSequence) {
      ...
      } while (carSequence[0] == carSequence[2] ||
               carSequence[1] == carSequence[2]);
    }
    do {
      ...
      positionInSequence = (positionInSequence + 1) % 3;
             ...
             cars[currentCarIndex].isOutOfGas());
