  ...
  for (int i = 1; i <= 3; i++) {
    for (int j = 0; j < 3; j++) {
      if (control.getPosDepart(j) == i) {
        ...
        QString name =
            QString("%1").arg(control.getDriverName(j), 25, QChar(' '));
        QString moves =
            QString("%1 moves").arg(control.getNbMoves(j), 3, 10, QChar(' '));
        QString gas =
            QString("%1 gas used").arg(consumption, 3, 10, QChar(' '));
        std::cout << i << " " << name.toStdString() << ' '
                  << moves.toStdString() << ' ' << gas.toStdString()
                  << std::endl;
      }
    }
  }
