...
GPMapView::GPMapView(QWidget *parent)
    : QWidget(parent), ui(new Ui::GPMapView), driverView(false), zoom(1.),
      normal(":/res/normal.png"), sand(":/res/sand.png"),
      goal(":/res/goal.png"), out(":/res/out.png"), car1(":/res/car1.png"),
      car2(":/res/car2.png"), car3(":/res/car3.png"),
      available1(":/res/available1.png"), available2(":/res/available2.png"),
      available3(":/res/available3.png"), current(":/res/current.png"),
      carToFollow(-1) {
  ui->setupUi(this);
...
void GPMapView::setCarToFollow(int nCar) { carToFollow = nCar; }
...
void GPMapView::paintEvent(QPaintEvent *event) {
  ...
  for (int y = minY; y < maxY; y++) {
    for (int x = minX; x < maxX; x++) {
      ...
      painter.drawImage(QRectF(offsetX + tileWidth * (x - minX),
                               offsetY + tileHeight * (y - minY),
                               tileWidth + 0.01, tileHeight + 0.01),
                        *img, QRectF(0, 0, 36, 36));
      if ((y - minY) == 0 && (driverView || x % 5 == 0)) {
        painter.drawText(QRectF(offsetX + tileWidth * (x - minX - 1), 0,
                                tileWidth * 3 + 0.01, offsetY),
                         QString("%1").arg(x), QTextOption(Qt::AlignCenter));
      }
      if ((x - minX) == 0 && (driverView || y % 5 == 0)) {
        painter.drawText(QRectF(0, offsetY + tileHeight * (y - minY - 1),
                                offsetX, tileHeight * 3 + 0.01),
                         QString("%1").arg(y), QTextOption(Qt::AlignCenter));
      }
    }
  }
  ...
  for (int noCar = 0; noCar < this->gpControl->getNbCars(); noCar++) {
    ...
    if (noCar == gpControl->getCurrentCar()) {
      painter.drawImage(QRectF(offsetX + tileWidth * (curCarX - minX),
                               offsetY + tileHeight * (curCarY - minY),
      ...
      if (gpControl->getCarDebug(noCar)) {
        ...
        for (i = available.begin(); i != available.end(); ++i) {
          painter.drawImage(
              QRectF(offsetX + tileWidth * (gpControl->getPosCarX(noCar) +
                                            gpControl->getVelCarX(noCar) +
                                            i->first - minX),
                     offsetY + tileHeight * (gpControl->getPosCarY(noCar) +
                                             gpControl->getVelCarY(noCar) +
                                             i->second - minY),
                     tileWidth, tileHeight),
              *img_available, QRectF(0, 0, 36, 36));
        }
      }
    }
    ...
    // Display of the cars
    if (img != NULL) {
      angle = qAtan2(gpControl->getVelCarX(noCar),
                     gpControl->getVelCarY(noCar)); // Radius between -Pi et Pi
      angle = angle * 180 / M_PI + 90;
      painter.translate(
          offsetX + tileWidth * (gpControl->getPosCarX(noCar) - minX) +
              tileWidth * 0.5,
          offsetY + tileHeight * (gpControl->getPosCarY(noCar) - minY) +
              tileHeight * 0.5); // On déplace le système de coordonnées au
    ...
    for (int i = 1; i < gpControl->getNbPositions(noCar); i++) {
      ...
      painter.drawLine(
          offsetX +
              tileWidth * (gpControl->getOldPosCarX(noCar, i - 1) - minX) +
              tileWidth * 0.5,
          offsetY +
              tileHeight * (gpControl->getOldPosCarY(noCar, i - 1) - minY) +
              tileHeight * 0.5,
          offsetX + tileWidth * (gpControl->getOldPosCarX(noCar, i) - minX) +
              tileWidth * 0.5,
          offsetY + tileHeight * (gpControl->getOldPosCarY(noCar, i) - minY) +
              tileHeight * 0.5);
    }
    ...
    for (int i = 0; i < (gpControl->getNbPositions(noCar) - 1); i++) {
      ...
      while (
          !boostUsed &&
          j < gpControl->getNbBoostUsed(
                  noCar)) { // Looking for a boost usage at the current position
      ...
      painter.drawEllipse(
          offsetX + tileWidth * (gpControl->getOldPosCarX(noCar, i) - minX) +
              tileWidth / 2 - widthCircle / 2,
          offsetY + tileHeight * (gpControl->getOldPosCarY(noCar, i) - minY) +
              tileHeight / 2 - widthCircle / 2,
