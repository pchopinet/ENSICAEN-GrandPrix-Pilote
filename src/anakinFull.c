...
int haveEnoughFuel(int fuelMax, float totalfuelConso) {
    int lowerFuel;
    lowerFuel = (totalfuelConso < 0.95f * (float) fuelMax);
    return lowerFuel;
}
...
int main() {
    ...
    while (!feof(stdin)) {
        ...
        if (a1.x != a2.x || a1.y != a2.y) {// Pour repartir en cas de crash
            ...
            if (!haveenoughFuel) {
                x = 0.5f * x;
                fprintf(log, "notEnoughFuel x = %.0f\n", x);
                fflush(log);
            }
        ...
        if ((a2.x == b.x && a2.y == b.y) || (a2.x == c.x && a2.y == c.y) ||
            !haveenoughFuel) {// pour ne pas se crash dans une autre voiture
